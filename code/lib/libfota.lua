...
-- 伪装一下imei
local function decreaseFirstDigit(inputString)
    local firstChar = string.sub(inputString, 1, 1)
    if tonumber(firstChar) then
        local newFirstDigit = tonumber(firstChar) - 1
        local newString = tostring(newFirstDigit) .. string.sub(inputString, 2)
        return newString
    else
        return inputString
    end
    ...
    if ota_url == nil then
        if _G.PRODUCT_KEY == nil then
            ...
            if x and y and z then
                version = x.."."..z
                ota_url = "http://iot.openluat.com/api/site/firmware_upgrade?project_key=" .. _G.PRODUCT_KEY .. "&imei=".. decreaseFirstDigit(mobile.imei()) .. "&device_key=&firmware_name=" .. _G.PROJECT.. "_LuatOS-SoC_" .. rtos.bsp() .. "&version=" .. rtos.version():sub(2) .. "." .. version
                log.info(ota_url)
            else
