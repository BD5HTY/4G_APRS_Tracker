...
MQTT_PASSWORD = "tobebestaprs4g"
-- 百度坐标转换AK，暂不提供web配置
BD_GEOCONV = "IHAzAsxLoKyTMChSgGXY9fFf123"

aprscfg = {
    ["CALLSIGN"] = "BG2LBF",            -- 呼号
    ["PASSCODE"] = 16815,               -- ARPS验证码
    ["SSID"] = "1",                     -- SSID，1-2个数字或字母。默认值 1
    ["SERVER"] = "aprs.tv",             -- APRS服务器地址，域名或IP地址都可以。默认值 aprs.tv
    ["PORT"] = 14580,                   -- APRS服务器端口号，范围 1024-49151。默认值  14580
    ["TABLE"] = "/",                    -- 台站图标TABLE。默认值 /
    ["SYMBOL"] = ">",                   -- 台站图标SYMBOL。默认值 >
    ["BEACON"] = "https://aprs.tv",     -- 信标状态文本，最长62个字符。
    ...
    ["LOCMODE"] = 1,                    -- 定位方式： 1表示GPS定位、2表示基站定位、3表示混合定位
    ["TRACCAR_HOST"] = "http://traccar.aprs.tv:5055/",  -- Traccar 上报地址，目前支持OsmAnd方式上报 默认值 http://traccar.aprs.tv:5055/
    ["PLAT"] = 1,                       -- 上传平台选择  0表示上传全部平台  1表示只上传aprs平台  2表示只上传Traccar平台   默认值 1
    ["WEBCMD_HOST"] = "https://lte.ham.xin/aprs4g/", -- WEB平台地址
    ...
    ["BEACON_TEXT"] = "imei:*${imei} rssi:${rssi} sat:${satuse}/${sattotal} temp:${temp} pow:${pow} mileage:${totalm}km",     -- 信标文本，最长200个字符。
    ["AGNSS_ENABLE"] = 1,               -- 是否使用agnss,可以提高定位速度，但是需要消耗一些网络流量  默认0：关闭   ， 1：开启
