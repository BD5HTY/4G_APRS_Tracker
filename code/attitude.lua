...
local QMA7981_stop_convert_cmd = 0x00  --设置QMA7981为standby模式的指令
local QMA7981_ANY_MOT_EN_cmd = 0x07  --设置QMA7981为运动模式的指令
local QMA7981_INT1_ANY_MOT_cmd = 0x01  --设置运动中断 0x01 
...
local QMA7981_D_TAP_EN_cmd = 0x20  --设置QMA7981为双击模式的指令
local QMA7981_INT1_D_TAP_cmd = 0x20  --设置双击中断 0x20 
...
local function initAttitude()
    ...
    -- sys.wait(100)
    -- i2c.writeReg(i2c_id, QMA7981_ADDR, QMA7981_start_INT_EN0_cmd,string.char(QMA7981_D_TAP_EN_cmd) )
    -- sys.wait(100)
    -- i2c.writeReg(i2c_id, QMA7981_ADDR, QMA7981_start_INT_MAP0_cmd,string.char(QMA7981_INT1_D_TAP_cmd) )
...
function stopAttitude()
    if USE_ATTITUDE == 1 then
        i2c.writeReg(i2c_id, QMA7981_ADDR, QMA7981_PWR_MGMT_1_REG_ADDR,string.char(QMA7981_stop_convert_cmd) )
    end
end
-- 判断是否为静态状态
...
sys.taskInit(function()
    ...
    -- gpio.setup(int_pin2, function(val) 
    --     log.info("发生双击", val)
