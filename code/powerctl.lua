...
local function checkPower()
    ...
    elseif tonumber(curPower) >= battery0 then
        ...
        return ((BATTERY_CHARGING == 1 and BATTERY_SOC < 0) or (BATTERY_CHARGING == 2 and BATTERY_SOC >= 0) or BATTERY_SOC == 0) and 1 or BATTERY_SOC
    end
end
	...
	elseif DEV_TYPE == "air780eg" then
        ...
        while true do
            ...
            elseif BATTERY_CHARGING == 2 and BATTERY_SOC <= 5 then
                sys.publish("AUDIO_POWER_LOW")
                if BATTERY_SOC == 1 then     -- 超低电 执行一次基站定位立即关机
                    sys.publish("LOW_POWER_SLEEP_NOW")
                end
            end
			sys.waitUntil("POWER_OFF_VOICE", 360000)
		end
...
sys.subscribe("SLEEP_NOW", function()
