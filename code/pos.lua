...
local agps_url = "http://download.openluat.com/9501-xingli/CASIC_data.dat"  -- http://download.openluat.com/9501-xingli/CASIC_data_bds.dat
-- 启用agnss辅助定位
    ...
    if http then
        ...
        while 1 do
            ...
            if (os.time() - agnsstime) > agnsslimit then 
                -- 网络加载
                local code, headers, body = http.request("GET", agps_url).wait()
...
local function initGps()
    ...
    elseif DEV_TYPE == "air780eg-yed" then
        uart.setup(gps_uart_id, 9600) -- p- 多模 9600    p- 北斗 115200
	    -- pm.power(pm.GPS, true)          -- GPS使能 ，请根据实际情况设置
...
sys.subscribe("LOW_POWER_SLEEP_NOW", function()
    table.insert(gpsTable, "LAST1MINS_STATIC_STATE_CLOSEGPS")
end)