...
sys.taskInit(function()
    ...
    sys.wait(6000)
    local req_data = {}
    req_data.devtype = DEV_TYPE
    req_data.imei = mobile.imei()
    req_data.version = VERSION
    req_data.callsign = aprscfg.CALLSIGN
    req_data.passcode = aprscfg.PASSCODE
    req_data.ssid = aprscfg.SSID
    req_data.serverip = aprscfg.SERVER
    req_data.serverport = aprscfg.PORT
    req_data.aprstable = aprscfg.TABLE
    req_data.aprssymbol = aprscfg.SYMBOL
    req_data.beacon = aprscfg.BEACON
    req_data.beacontext = aprscfg.BEACON_TEXT
    req_data.beaconinterval = aprscfg.BEACON_INTERVAL
    req_data.beaconstatusinterval = aprscfg.BEACON_STATUS_INTERVAL
    req_data.trackermode = aprscfg.TRACKERMODE
    req_data.fixloclat = aprscfg.FIXLOC_LAT
    req_data.fixloclng = aprscfg.FIXLOC_LNG
    req_data.locmode = aprscfg.LOCMODE
    req_data.traccarhost = aprscfg.TRACCAR_HOST
    req_data.plat = aprscfg.PLAT
    req_data.platsendself = aprscfg.PLAT_SENDSELF
    req_data.webcmdhost = aprscfg.WEBCMD_HOST
    req_data.maxdecay = aprscfg.MAX_DECAY
    req_data.trickmode = aprscfg.TRICK_MODE
    req_data.travelmode = aprscfg.TRAVEL_MODE
    req_data.displayver = aprscfg.DISPLAY_VER
    req_data.agnssenable = aprscfg.AGNSS_ENABLE
    req_data.audioenable = aprscfg.AUDIO_ENABLE
    req_data.audiovol = aprscfg.AUDIO_VOL
    req_data.audiovoiceprompt = aprscfg.AUDIO_VOICE_PROMPT
    req_data.powersavemode = aprscfg.POWER_SAVE_MODE
    req_data.powersavemodetime = aprscfg.POWER_SAVE_MODE_TIME
    req_data.powersavemoded1time = aprscfg.POWER_SAVE_MODE_D1TIME
    req_data.powersavemoded2time = aprscfg.POWER_SAVE_MODE_D2TIME
    req_data.smscatenable = aprscfg.SMS_CAT_ENABLE
    req_data.smscatmobile = aprscfg.SMS_CAT_MOBILE
    req_data.totalmileage = tonumber(string.format("%07.2f", aprscfg.TOTAL_MILEAGE))
    req_data.totalmileageautoclear = aprscfg.TOTAL_MILEAGE_AUTOCLEAR
    req_data.shicitype = aprscfg.SHICI_TYPE
    req_data.talkmode = aprscfg.TALK_MODE
    req_data.talkchannel = aprscfg.TALK_CHANNEL
    req_data.btnmode = aprscfg.BTN_MODE
    req_data.powerrestarttime = aprscfg.POWER_RESTART_TIME
    req_data.coordenc = aprscfg.COORD_ENC
    req_data.pwkmode = aprscfg.PWK_MODE
    local code = aprs_upload_conf(req_data)
    if code == 200 then
        sys.wait(500)
        local code2, body2 = aprs_update_conf(req_data.imei)
        if code2 == 200 then
            sys.wait(500)
            local bTable = json.decode(body2)
            if bTable then -- 若解码失败, 会返回nil
                if bTable.retCode == "0000" then
                    if iniChk(bTable.body) then
                        log.info("web指令", "准备配置更新")
                        changeAprsConf(bTable.body)
                    else
                        log.error("web指令", "参数校验失败")
                    end
                else
                    log.info("web指令", "无配置更新")
                end
                   
            end
        else
            log.info("web指令", "无配置更新2")
        end
    else
        log.error("web指令", "网络通信失败")
    end
end)