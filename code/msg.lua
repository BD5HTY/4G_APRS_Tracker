...
local function point2msg(pointData)
    ...
    if pointData.gps then
        ...
        return string.format("%s>APLZA2:/%02d%02d%02dh%07.2f%s%s%08.2f%s%s%03d/%03d/A=%06d %s%s",
        ...
        -- LBS
        return string.format("%s>APLZA2:/%02d%02d%02dh%07.2f%s%s%08.2f%s%s %s%s",
...
local function point2traccarmsg(pointData)
    return string.format("?lat=%s&id=%s&lon=%s&timestamp=%s&hdop=%s&altitude=%s&speed=%s", 
             convertCoordinates(pointData.lat),fullImei, convertCoordinates(pointData.lng), os.time().."000", pointData.hdop,  string.format("%.2f", pointData.alt/3.2808399), pointData.spd)
end
...
local function forwardpoint2traccarmsg(pointData)
    if aprscfg.SMS_CAT_ENABLE == 4 then
        return string.format("?lat=%s&id=%s&lon=%s&timestamp=%s&hdop=%s&altitude=%s&speed=%s", 
             convertCoordinates(pointData.lat),fullImei, convertCoordinates(pointData.lng), os.time().."000", 1,  0, 0)
    else
        return string.format("?lat=%s&id=%s&lon=%s&timestamp=%s&hdop=%s&altitude=%s&speed=%s", 
             convertCoordinates(pointData.lat),pointData.call, convertCoordinates(pointData.lng), os.time().."000", 1,  0, 0)
    end 
end
-- 转发的aprs消息组装
local function forwardpoint2msg(pointData)
    ...
    if aprscfg.SMS_CAT_ENABLE == 4 then
        ...
        return string.format("%s>APLZA2,TCPIP*,qAS,%s:/%02d%02d%02dh%07.2f%s%s%08.2f%s%s %s%s",
        ...
        if aprscfg.PLAT == 0 or aprscfg.PLAT == 1 then
            ...
            if aprscfg.BEACON_STATUS_INTERVAL ~= 0 and os.time() - beaconTime >= aprscfg.BEACON_STATUS_INTERVAL then
                ...
                if aprscfg.DISPLAY_VER == 1 then
                    beaconMsg = string.format("%s>APLZA2:>%s %s", aprscfg.sourceCall, templateRendering(aprscfg.BEACON), VERSION)
                else
                    beaconMsg = string.format("%s>APLZA2:>%s", aprscfg.sourceCall, templateRendering(aprscfg.BEACON))
                end
                beaconTime = os.time()
